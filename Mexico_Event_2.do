...
use "$mxfls2data/hh05dta_b3a/iiia_tb",clear

preserve
tempfile sex
use "$mxfls2data/hh05dta_bc/c_ls",clear
rename ls04 sex
rename ls02_2 age
...
// income info
merge 1:1 folio ls using "$mxfls2data/hh05dta_bc/c_portad"
rename _merge income_merge


// pregnancy history
merge 1:1 folio ls using "$mxfls2data/hh05dta_b4/iv_res"
rename _merge pregnancy_merge

// birth history
merge 1:1 folio ls using "$mxfls2data/hh05dta_b4/iv_he"
rename _merge birth_history_merge

// first birth info
preserve
tempfile first_birth
use "$mxfls2data/hh05dta_b4/iv_he1",clear
...
// folio: hhid; ls: indid
bysort folio ls: egen fbage = min(he07a)
...
// education info
merge 1:1 folio ls using "$mxfls2data/hh05dta_b3a/iiia_ed"
rename _merge education_merge

// control info
preserve
tempfile control
use "$mxfls2data/hh05dta_b4/iv_conpor",clear
...
replace nochild = 1 if res00a == 3 & res01a == 3 // no pregnancy 
replace nochild = 0 if res00a == 1 & he01_1a > 0 // pregnancy before 2015 and have kid
replace nochild = 0 if res01a == 1 & res01 == 1 // pregnancy after 2015 and have kid
replace nochild = 1 if res00a == 1 & res01a == 1 & he01_1a == 0 & res01 == 3 // have pregnancy but no child

// first birth age
// see above: pregnancy history and birth history

// keep short-listed variables
keep folio ls pid_link age* year ///
...
rename pid_link_2 pid_link

gen folio = real(folio_2)
gen ls = real(ls_2)

drop folio_2 ls_2
save "$data/Mexico_cleaned_2.dta",replace
